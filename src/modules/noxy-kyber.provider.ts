...
import { NoxyKyberProviderError } from '@/modules/noxy-error';
...
export class NoxyKyberProvider {
  ...
  private static instance: Promise<NoxyKyberProvider> | undefined = undefined;
  ...
  static async create(): Promise<NoxyKyberProvider> {
    ...
    NoxyKyberProvider.instance = (async () => {
      ...
      const mod = await KyberModule({
        ...(globalBinary && { wasmBinary: globalBinary }),
        locateFile: (path: string, scriptDirectory: string) =>
          new URL(path, scriptDirectory).href,
      });
  ...
  private assertReady() {
    if (!this.mod) {
      throw new NoxyKyberProviderError({ code: 'KYBER_PROVIDER_NOT_INITIALIZED', message: 'KyberProvider not initialized' });
    }
  }
  ...
  private assertSize(
    ...
    if (buf.length !== expected) {
      throw new NoxyKyberProviderError(
        { code: 'KYBER_PROVIDER_INVALID_SIZE', message: `${name} must be ${expected} bytes, got ${buf.length}` }
      );
    }
  }
}