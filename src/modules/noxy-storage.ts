...
import { NOXY_LOCAL_STORAGE_CONFIG } from '@/modules/noxy-storage.types';
import type {
  NoxyStorageOptions,
...
export class NoxyStorageModule implements NoxyLocalStorageAPI {
  ...
  constructor(options: NoAny<NoxyStorageOptions> = {}) {
    ...
    this.#dbEncryptionKey = options.encryptionKey ?? randomBytes(32);
  }
  ...
  private async validateDbEncryptionKey(): Promise<void> {
    ...
    if (!validKeySizes.includes(this.#dbEncryptionKey.byteLength)) {
      throw new NoxyStorageError({
        code: 'INVALID_ENCRYPTION_KEY',
        message: `Invalid db encryption key length. AES key must be 16, 24, or 32 bytes for CBC/CTR/GCM modes, got ${this.#dbEncryptionKey.byteLength} bytes`,
  ...
  async loadDecrypted(
    ...
    return webcrypto.gcm(this.#dbEncryptionKey, record.iv).decrypt(record.ciphertext);
  }
  ...
  async saveEncrypted(
    ...
    const ciphertext = await webcrypto.gcm(this.#dbEncryptionKey, iv).encrypt(data);
